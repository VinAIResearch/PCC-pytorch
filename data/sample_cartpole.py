...
def sample(sample_size=15000, width=80, height=80, frequency=50, noise=0.0):
    ...
    """
    mdp = CartPoleMDP(width=width, height=height,
    ...
    for sample in trange(sample_size, desc = 'Sampling data'):
        ...
        state_data[sample, :, 0] = s0[0:4]
        state_data[sample, :, 1] = s1[0:4]
        # Action.
        u_data[sample] = a1
        # Next state (w/ history).
        x_next_data[sample, :, :, 0] = x1[:, :, 0]
        x_next_data[sample, :, :, 1] = x2[:, :, 0]
        state_next_data[sample, :, 0] = s1[0:4]
        state_next_data[sample, :, 1] = s2[0:4]
...
def write_to_file(sample_size, noise):
    ...
    """
    output_dir = root_path + '/data/cartpole/raw_{:d}_{:.0f}'.format(sample_size, noise)
