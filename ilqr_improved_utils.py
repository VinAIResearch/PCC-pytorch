...
def update_state(state, action, config):
    pass
...
def get_x_data(mdp, state, config):
    ...
    if config['task'] == 'plane':
        ...
        # x_dim = np.prod(x_dim)
        # image_data = np.hstack((image_data, image_data))
        # x_data = Image.fromarray(image_data * 255.).convert('L')
        # x_data = ToTensor()(x_data.convert('L').resize((96, 48))).double()
        # x_data = torch.cat((x_data[:, :, :48], x_data[:, :, 48:]), dim=1).view(-1, x_dim)
...
def traj_opt_actions(s_start, image_start, actions_final, mdp, env_name):
    goal_counter = 0.0
    s = s_start
    ...
    for i, u in enumerate(actions_final):
        ...
        if env_name == 'planar':
            ...
            s, image = mdp.transition_function((s, obs_traj[-1]), u)
            obs_traj.append(image.squeeze())
            goal_counter += mdp.reward_function((s, image))
