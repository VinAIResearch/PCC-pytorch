...
class ThreePoleMDP(PoleBase):
    ...
    def __init__(self, width=80, height=80, frequency=50,
                 noise=0.0, torque=1.0, line_width=5):
        """
        Args:
    ...
    def ds_dt(self, t, s_augmented):
        ...
        i_1 = 1. / 3. * self.mass_pend_1 * self.length_1 ** 2
        i_2 = 1. / 3. * self.mass_pend_2 * self.length_2 ** 2
        i_3 = 1. / 3. * self.mass_pend_3 * self.length_3 ** 2
        ...
        alpha_3 = (self.mass_pend_2 * length_c2 +
                   self.mass_pend_3 * self.length_2) * self.length_1
        alpha_4 = i_3
        ...
        h_32 = h_23
        h_33 = alpha_4
        h_mat = np.array([[h_11, h_12, h_13], [h_21, h_22, h_23],
                          [h_31, h_32, h_33]])

        beta_1 = (self.mass_pend_1 * length_c1 + self.mass_pend_2 * self.length_1 +
                  self.mass_pend_3 * self.length_1) * self.earth_gravity
        beta_2 = (self.mass_pend_2 * length_c2 +
                  self.mass_pend_3 * self.length_2) * self.earth_gravity
        beta_3 = self.mass_pend_3 * self.earth_gravity * length_c3

        c_11 = alpha_5 * (
                theta_2_dot + theta_3_dot
        ) * sine_2_3 + alpha_3 * theta_2_dot * sine_2 + alpha_6 * theta_3_dot * sine_3
        c_12 = alpha_5 * (
                theta_1_dot + theta_2_dot + theta_3_dot) * sine_2_3 + alpha_3 * (
                       theta_1_dot + theta_2_dot) * sine_2 + alpha_6 * theta_3_dot * sine_3
        c_13 = (theta_1_dot + theta_2_dot + theta_3_dot) * (
                alpha_5 * sine_2_3 + alpha_6 * sine_3)
        ...
        c_31 = -alpha_5 * theta_1_dot * sine_2_3 - alpha_6 * (theta_1_dot +
                                                              theta_2_dot) * sine_3
        c_32 = -alpha_6 * (theta_1_dot + theta_2_dot) * sine_3
        c_33 = 0.0
        c_mat = np.array([[c_11, c_12, c_13], [c_21, c_22, c_23],
                          [c_31, c_32, c_33]])
        ...
        # Derivatives.
        return np.array([
            theta_1_dot, theta_1_double_dot, theta_2_dot, theta_2_double_dot,
            theta_3_dot, theta_3_double_dot, 0.0, 0.0, 0.0
        ])
    ...
    def render(self, s):
        ...
        # Draw pole 2.
        draw.line((xstart_2, ystart_2, xend_2, yend_2),
                       width=self.line_width,
                       fill=255)
        ...
        # Draw pole 2.
        draw.line((xstart_3, ystart_3, xend_3, yend_3),
                       width=self.line_width,
                       fill=255)
    ...
    def sample_random_state(self):
        ...
        angle_1_rate = np.random.uniform(self.angular_velocity_range[0],
                                         self.angular_velocity_range[1])
        angle_2 = np.random.uniform(self.angle_2_range[0], self.angle_2_range[1])
        angle_2_rate = np.random.uniform(self.angular_velocity_range[0],
                                         self.angular_velocity_range[1])
        angle_3 = np.random.uniform(self.angle_3_range[0], self.angle_3_range[1])
        angle_3_rate = np.random.uniform(self.angular_velocity_range[0],
                                         self.angular_velocity_range[1])
        true_state = np.array(
            [angle_1, angle_1_rate, angle_2, angle_2_rate, angle_3, angle_3_rate])
        return true_state