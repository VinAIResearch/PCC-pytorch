...
def reciding_horizon(env_name, mdp, x_dim, R_z, R_u, s_start, z_start, z_goal, dynamics, encoder, iters_ilqr, horizon):
    ...
    for i in range(horizon):
        ...
        if env_name == 'planar':
            ...
            s_next = mdp.transition_function(s, u_first_opt)
            next_obs = np.hstack((image, mdp.render(s_next).squeeze()))
            ...
            image = mdp.render(s).squeeze()
            s, next_image = mdp.transition_function(s, u_first_opt)
